##################################################################
#       在transormer增加更多的CNN提取特征(3layer)
##################################################################
...
class DispersionTransformer(nn.Module):
    def __init__(self, model_dim, num_heads, num_layers, output_dim, device=torch.device('cpu')):
        ...
        self.conv_fusion1 = nn.Conv1d(model_dim * 3, model_dim, kernel_size=3, padding=1).to(device)
        self.conv_fusion2 = nn.Conv1d(model_dim    , model_dim, kernel_size=3, padding=1).to(device)
        self.conv_fusion3 = nn.Conv1d(model_dim    , model_dim, kernel_size=3, padding=1).to(device)
    ...
    def forward(self, input_data, mask=None):
        ...
        fused_features = self.conv_fusion1(combined_embedding)  # (batch_size, model_dim, seq_length)
        fused_features = self.conv_fusion2(fused_features)      # (batch_size, model_dim, seq_length)
        fused_features = self.conv_fusion3(fused_features)      # (batch_size, model_dim, seq_length)
