...
##################################################################
#       position embedding + linear position embedding + 末尾多层FNN进行映射(效果不好)
##################################################################
...
class DispersionTransformer(nn.Module):
    def __init__(self, model_dim, num_heads, num_layers, output_dim, device=torch.device('cpu')):
        ...
        self.fc_layers = nn.Sequential(
            ...
            nn.Linear(model_dim,1024).to(device),
            nn.ReLU(),
            nn.Dropout(0.0),
    ...
    def forward(self, input_data, mask=None):
        ...
        period_embedding            = self.period_encoding(period_data.unsqueeze(-1))
        ...
        output = self.fc_layers(pooled_output)  # (batch_size, seq_length)
